...
#  Description: This program tests a variety of recursive functions for a given
#               problem statement. 
...
def bunnyEars2(bunnies):
    if bunnies == 0:
        ...
        if bunnies % 2 == 0:      #when bunny is even number
            return 3 + bunnyEars2(bunnies -1 )
        if bunnies % 2 == 1:      #when bunny is odd number
...
def count7(n):
    if n == 0:
        return 0
    if n%10 == 7:
        return 1 + count7(n//10)   #counts 1 and move to next number
    else:
        return count7(n//10)   #move on to the next number
...
def count8(n):
    if n == 0:
        return 0
    elif n%10 == 8 and (n//10)%10 == 8:   #when right and left digit = 8
        return 2 + count8(n//10)
    elif n%10 == 8:                 #when only the right digit = 8
...
def countX(str):
    if str == '':
        return 0
    elif str[0] == 'x':         #count 1 and return the rest of the string 
...
def countHi(str):
    if len(str) < 2:
        return 0
    if str[0] == 'h' and str[1] == 'i':   #count 1 and return the rest of string
...
def changeXY(str):
    if str == '':
        return ''
    if str[0] == 'x':           #identifies all the 'x's 
        return 'y' + changeXY(str[1:])
    else:                       #return original character if not 'x'
...
def changePi(str):
    ...
    if str[0] == 'p' and str[1] != 'i':
        return str[0] + changePi(str[1:])
    if str[0] == 'p' and str[1] == 'i':     #replace char with 3.14
...
def noX(str):
    if str == '':
        return str
    elif str[0] != 'x':    #only concatenates non-'x' characters
...
def array11(nums, index):
    if len(nums) <= index:
        return 0
    elif nums[index] == 11:         #counts 1 when int = 11 and returns rest of ints
...
def array220(nums, index):
    if len(nums)-1 <= index :
        return False
    elif nums[index]*10 == nums[index+1]:  #check to see if adjacent value is
        return True                         # 10 times before
    else:
...
def pairStar(str):
    if len(str) < 2:
        return str
    elif str[0] == str[1]:   #insert * when adjacent char are same
...
def endX(str):
    if str == '':
        return str
    elif str[0] == 'x':         #moves 'x' to the end of string
        return endX(str[1:]) + 'x'
    else:
...
def countPairs(str):
    if len(str) < 3:
        return 0
    elif str[0] == str[2]:              #counts 1 when pair is identified 
...
def countAbc(str):
    if len(str) < 3:
        return 0
    elif (str[0] == 'a' and str[1] == 'b') and (str[2] == 'c' or str[2] == 'a'):
        return 1 + countAbc(str[1:]) #counts 1 when 'abc' or 'aba' are identified
    else:
...
def count11(str):
    if len(str) < 2:
        return 0
    elif str[0] == '1' and str[1] == '1':   #counts 1 when 11 is identified in str
...
def stringClean(str):
    if len(str) < 2:
        ...
        return str[0] + stringClean(str[1:])  #concatenates non-duplicated char
...
def countHi2(str):
    if len(str) < 2:
        return 0
    elif str[0] == 'x' and str[1] == 'h' and str[2] == 'i': #moves past 'hi' if 'x' is before it
        return countHi2(str[3:])
    elif str[0] == 'h' and str[1] == 'i': #counts 1 when there is no 'x' before 'hi'
...
def parenBit(str):
   if str[0] == '(' and str[-1] == ')':
       ...
       return parenBit(str[:-1])   #removes trailing strings until first and last char are parentheses
   elif str[-1] == ')':
...
def nestParen(str):
    if len(str) == 0:
        return True
    elif str[0] == '(' and str[-1] == ')':  #recursively checks the first and last chars for parentheses
...
def strCount(str, sub):
    if len(str)< len(sub):
        return 0
    elif str[0:len(sub)] == sub:  #counts 1 when substring matches string 
...
def strCopies(str, sub, n):
    if n == 0:
        ...
        return False
    elif str[:len(sub)] == sub:         #decreases n if substring is found 
...
def strDist(str, sub):
    if len(str) == 0:
        return 0
    elif str[0:len(sub)] == str[-len(sub):] == sub:
        return len(str)
    elif str[0:len(sub)] != sub and str[-1:-len(sub)-1:-1] != sub:  #reduces string until it starts and ends with sub and returns length
